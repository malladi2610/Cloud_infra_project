...
usage() {
  cat <<'EOF'
Usage:
  run_sync_benchmark.sh --email <email> --password <password> [options]
...
Options:
  ...
  --runs <n>                    Number of sync jobs to submit (default: 8)
  --poll-interval-sec <n>       Poll interval in seconds (default: 2)
  --timeout-sec <n>             Max total wait in seconds (default: 900)
...
Examples:
  ...
  run_sync_benchmark.sh \
    ...
    --runs 8 \
    --output-json /tmp/sync_benchmark.json
EOF
}
...
echo "[3/6] Submit ${RUNS} sync jobs"
for i in $(seq 1 "$RUNS"); do
  run_payload="$(jq -nc \
    --arg profileId "$PROFILE_ID" \
    --arg documentId "$DOCUMENT_ID" \
    '{profileId:$profileId, documentId:$documentId, executionMode:"sync", batchStrategy:"count_only"}')"
...
for job_id in "${job_ids[@]}"; do
  detail_response="$(json_request "GET" "/api/summaries/jobs/${job_id}")"
  echo "$detail_response" | jq -c '.job' >>"$JOB_FINAL_FILE"
done
  ...
  jq -s --arg app_url "$APP_URL" --arg profile_id "$PROFILE_ID" --arg document_id "$DOCUMENT_ID" '
    ...
    | {
        ...
        averagesCompleted: {
          ...
          latencyMs: (if ($latencies | length) == 0 then 0 else (($latencies | add) / ($latencies | length)) end),
